function [D,Gamma,err,gerr] = kcpd(params,varargin)
...
maxsignals = 2000;   % 限制最大样本数，当样本多余1万时cp_als会很慢，多余2万时几乎停止
...
%     gamma_j = reshape(P.lambda*P.U{4},1, length(data_indices));
    ...
    gamma_j = sum(Xvect(:,data_indices).*repmat(atomvect,1,length(data_indices)));% 内积
else
