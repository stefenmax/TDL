...
% which is used to generate parameters and training data


function [params,verbose,msgdelta] = parasetCPD(params,msgdelta)
...
if (isfield(params,'initdict') && ischar(params.initdict))
  if (strcmpi(params.initdict,'odct'))
    params.initdict = odctndict(tensorblocksize,dictsize,p);
...
params.Edata = sqrt(prod(tensorblocksize)) * sigma * gain;   % target error for omp
...
ids = cell(p,1);
if (p==1)
  ids{1} = reggrid(length(x)-tensorblocksize+1, trainnum, 'eqdist');
else
  [ids{:}] = reggrid(size(x)-tensorblocksize+1, trainnum, 'eqdist');
end
% params.data = sampgrid(x,blocksize,ids{:});
params.data = sampgridCPD(x,tensorblocksize,ids{:});

% remove DC (means) in blocks 
params.data = remove_dcCPD(params.data,params.dctype,tensorblocksize);
...
% end
% params.lambda = 0.45 * 144 / sigma;
params.lambda = 0.45 * 64 / sigma;
