...
function [A]=myOMPerr_group(D,X,ompparams); 
...
for k=1:1:P, %按信号顺序
    a=[];
    x = zeros(n,nMC); % 将一个信号的数据重排成通道的形式
    for iMC = 1:nMC
        x(:,iMC)=X((iMC-1)*n+1:iMC*n,k); %第k个信号的第iMC个通道
    end
    residual=x;
	indx = [];
	a = [];
	...
	j = 0;
    while max(sqrt(currResNorm2)-E2)>0 & j < maxNumCoef, % 对第k个信号只要某个通道残差大于相应阈值或编码最多更新maxNumCoef次
		j = j+1;
        proj=D'*residual;
        projabssum = sum(abs(proj),2); % 求各个通道表达系数绝对值总和
        pos=find(projabssum==max(projabssum)); % 找到proj中绝对值最大的一项的索引
        pos=pos(1);
        indx(j)=pos; % 
